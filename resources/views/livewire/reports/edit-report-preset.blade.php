{{-- resources/views/livewire/reports/edit-report-preset.blade.php --}}
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            <div class="mb-4">
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">
                    Seleziona un report per modificarne configurazione, filtri fissi e vista predefinita.
                </p>
            </div>
            ...
            <div class="space-y-3">
                @forelse ($reportPresets as $reportPreset)
                    <button
                        type="button"
                        wire:key="edit-report-preset-{{ $reportPreset['id'] }}"
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                    {{ $this->getReportTypeLabel($reportPreset['report_type']) }}
                                </div>
                            </div>
                        </div>
                    </button>
                @empty
                    <div class="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-600 dark:border-gray-600 dark:text-gray-300">
                        Non ci sono ancora report salvati da modificare.
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="lg:col-span-8">
        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            <div class="mb-4">
                ...
                @if ($successMessage)
                    <div class="mt-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700 dark:border-green-800 dark:bg-green-900/20 dark:text-green-300">
                        {{ $successMessage }}
                    </div>
                @endif
            </div>

            @if ($selectedReportPresetId)
                <form wire:submit="save" class="space-y-6">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label for="edit_preset_name" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                                Nome del report
                            </label>
                        ...
                        <div>
                            <label for="edit_report_type" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                                Tipo di analisi
                            </label>
                    ...
                    <div>
                        <label for="edit_description" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                            Descrizione
                        </label>
                    ...
                    <div class="space-y-4">
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-300">
                            Questi filtri resteranno salvati nel report. Il periodo verrà invece scelto ogni volta al momento del lancio.
                        </p>

                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            @if (in_array('organization_id', $availableFilters, true))
                                <div class="relative">
                                    <label for="edit_organization_search" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                                    ...
                                    @if (! empty($organizationOptions))
                                        <div class="mt-2 max-h-56 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                                            @foreach ($organizationOptions as $organizationOption)
                                                <button
                                                    type="button"
                                                    wire:key="edit-organization-option-{{ $organizationOption['id'] }}"
                            ...
                            @if (in_array('vehicle_id', $availableFilters, true))
                                <div class="relative">
                                    <label for="edit_vehicle_search" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                                    ...
                                    @if (! empty($vehicleOptions))
                                        <div class="mt-2 max-h-56 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                                            @foreach ($vehicleOptions as $vehicleOption)
                                                <button
                                                    type="button"
                                                    wire:key="edit-vehicle-option-{{ $vehicleOption['id'] }}"
                            ...
                            @if (in_array('rental_id', $availableFilters, true))
                                <div class="relative">
                                    <label for="edit_rental_search" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                                    ...
                                    @if (! empty($rentalOptions))
                                        <div class="mt-2 max-h-56 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                                            @foreach ($rentalOptions as $rentalOption)
                                                <button
                                                    type="button"
                                                    wire:key="edit-rental-option-{{ $rentalOption['id'] }}"
                            ...
                            @if (in_array('payment_method', $availableFilters, true))
                                <div>
                                    <label for="edit_payment_method" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                            ...
                            @if (in_array('kind', $availableFilters, true))
                                <div>
                                    <label for="edit_kind" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
                            ...
                            @if (in_array('is_commissionable', $availableFilters, true))
                                <div>
                                    <label for="edit_is_commissionable" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">
            ...
            @else
                <div class="rounded-lg border border-dashed border-gray-300 p-4 text-sm text-gray-600 dark:border-gray-600 dark:text-gray-300">
                    Seleziona un report dalla colonna di sinistra per modificarlo.
                </div>
            @endif
        </div>
    </div>
