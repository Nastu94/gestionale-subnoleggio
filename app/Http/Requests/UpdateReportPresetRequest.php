...
/**
 * Request per l'aggiornamento di un preset report.
 *
 * Responsabilità:
 ...
 * - gestire correttamente l'unicità del nome sul record corrente;
 * - escludere dal salvataggio i filtri runtime (date_from/date_to).
 */
class UpdateReportPresetRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        /** @var \App\Models\ReportPreset|null $reportPreset */
        $reportPreset = $this->route('reportPreset');
        ...
        return [
            ...
            'name' => [
                ...
                Rule::unique('report_presets', 'name')
                    ->where(fn ($query) => $query->where('created_by', $this->user()->id))
                    ->ignore($reportPreset?->id),
            ],
    ...
    /**
     * Restituisce i dati validati e normalizzati per l'aggiornamento del preset.
     *
     * @return array<string, mixed>
     */
